...
/***/ function(module, exports, __webpack_require__) {
	...
	var App = function (_Component) {
	  ...
	  _createClass(App, [{
	    key: 'toggleShowMoves',
	    value: function toggleShowMoves() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _board_explorer = __webpack_require__(655);

	var _board_explorer2 = _interopRequireDefault(_board_explorer);

	var _tile = __webpack_require__(654);
	...
	var Board = function (_Component) {
		...
		function Board(props) {
			...
			_this.state = {
				board_state: _board_explorer2.default.getInitialBoardState(),
				selected_tile: "",
				selected_piece: ""
			};
			return _this;
		...
		_createClass(Board, [{
			key: 'setSelectedTile',
			value: function setSelectedTile(tile, piece) {
				this.setState({
					selected_tile: tile,
			...
			value: function getHighlights() {
				...
				var selected_tile = _state.selected_tile;
				var selected_piece = _state.selected_piece;
				var board_state = _state.board_state;

				var showMoves = this.props.showMoves;
				if (!showMoves || selected_tile == "" || selected_piece == "0") {
					return [];
				} else {
					return _board_explorer2.default.getEligibleMoves(selected_tile, selected_piece, board_state);
				}
			}
		}, {
			key: 'renderTiles',
			value: function renderTiles() {
				...
				var range = _tools2.default.range(64);
				var tiles = range.map(function (index) {
					var tileName = _tools2.default.getTileName(rowIndex, columnIndex);
					var props = {
						setSelectedTile: _this2.setSelectedTile.bind(_this2),
						selectedTile: _this2.state.selected_tile,
						highlight: highlights.indexOf(tileName) > -1,
						name: tileName,
						piece: _this2.state.board_state[index],
						dark: rowIndex % 2 != columnIndex % 2,
						key: tileName
					};

					if (columnIndex < 7) columnIndex++;else columnIndex = 0;
					if (columnIndex == 0) rowIndex++;

					return _react2.default.createElement(_tile2.default, props);
				});
...
/***/ function(module, exports) {
	...
	var Tools = function () {
		...
		_createClass(Tools, null, [{
			key: 'getTileName',
			value: function getTileName(i, j) {
				var letters = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'];
				return letters[j] + (8 - i);
			}
		}, {
			key: 'getTileCoords',
			value: function getTileCoords(name) {
				var letters = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'];
				return [8 - name.split("")[1], letters.indexOf(name.split("")[0])];
			}
		}, {
			key: 'getPiece',
			value: function getPiece(tile, boardState) {
				var coords = typeof tile == "string" ? this.getTileCoords(tile) : tile; // ta ao contrario
				var boardStateIndex = coords[0] * 8 + coords[1];
				return boardState[boardStateIndex];
			}
		}, {
			key: 'getPlayer',
			value: function getPlayer(piece) {
				var whitePieces = ['t', 'n', 'b', 'q', 'k', 'p'];
				var blackPieces = ['T', 'N', 'B', 'Q', 'K', 'P'];
			...
			key: 'getBoardArray',
			value: function getBoardArray(boardState) {
				var counter = 0;
				var boardArray = [];
			...
			key: 'getPieceImage',
			value: function getPieceImage(piece) {
				switch (piece) {
					case "t":
						return "wr";
					case "n":
	...
	var Tile = function (_Component) {
		...
		_createClass(Tile, [{
			...
			value: function onTileClick() {
				...
				var selectedTile = _props.selectedTile;
				var name = _props.name;
				var piece = _props.piece;

				if (selectedTile == "") this.props.setSelectedTile(name, piece);else if (selectedTile == name) this.props.setSelectedTile("", "");else {}
			}
		}, {
			key: 'renderPiece',
			value: function renderPiece() {
				...
				if (piece != "0") {
					var filePath = "../assets/" + _tools2.default.getPieceImage(piece) + ".png";
					return _react2.default.createElement('img', { className: 'piece_image', src: filePath });
				}
			}
		}, {
			key: 'renderHightlight',
			value: function renderHightlight() {
				if (this.props.highlight) {
					return _react2.default.createElement('div', { id: 'highlight' });
				}
			}
		}, {
			key: 'render',
			value: function render() {
				...
				var selectedTile = _props2.selectedTile;
				var name = _props2.name;
				var dark = _props2.dark;

				var tileColor = (dark ? "dark" : "light") + (selectedTile == name ? "_selected" : "");
				return _react2.default.createElement(
					'div',
					{ className: tileColor, id: 'tile', onClick: this.onTileClick.bind(this) },
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _chess_engine = __webpack_require__(657);

	var _chess_engine2 = _interopRequireDefault(_chess_engine);
	...
	var BoardExplorer = function () {
		function BoardExplorer() {
			_classCallCheck(this, BoardExplorer);
		}

		_createClass(BoardExplorer, null, [{
			key: 'getInitialBoardState',
			value: function getInitialBoardState() {
			...
			key: 'getEligibleMoves',
			value: function getEligibleMoves(selectedTile, selectedPiece, boardState) {
				var moves = _chess_engine2.default.getMoves(selectedPiece, selectedTile, boardState);

				return moves.map(function (move) {
					return _tools2.default.getTileName(move.i, move.j);
				});
			}
		}]);
		...
		_createClass(Menu, [{
			...
			value: function render() {
				...
				return _react2.default.createElement(
					...
					_react2.default.createElement(
						"div",
						{ className: "menu_button", id: "new_game", onClick: this.newGame.bind(this) },
						" NEW GAME "
					),
					_react2.default.createElement(
						"div",
						{ className: "menu_button" + (showMoves ? " active" : ""), id: "show_moves", onClick: this.showMoves.bind(this) },
						" SHOW MOVES "
					)
				);
			}
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ChessEngine = function () {
		function ChessEngine() {
			_classCallCheck(this, ChessEngine);
		}

		_createClass(ChessEngine, null, [{
			key: "getMoves",
			value: function getMoves(piece, tile, state) {
				switch (piece.toUpperCase()) {
					case "P":
						return this.pawnMoves(tile, state, piece);
					default:
						return [];
				}
			}
			...
			key: "pawnMoves",
			value: function pawnMoves(tile, state, piece) {
				var coords = _tools2.default.getTileCoords(tile);
				var i = coords[0];
				var j = coords[1];
				var attack = _tools2.default.getPlayer(piece);
				var defence = attack == "white" ? "black" : "white";
				var front = attack == "white" ? i - 1 : i + 1;
				var twoInFront = attack == "white" ? i - 2 : i + 2;
				var left = j - 1;
				var center = j;
				var right = j + 1;
				var boardArray = _tools2.default.getBoardArray(state);
				...
				if (front >= 0 && front < 8) {
					...
					if (left >= 0 && _tools2.default.getPlayer(boardArray[front][left]) == defence) {
						pawnMoves.push({ i: front, j: left });
					}
					if (right < 8 && _tools2.default.getPlayer(boardArray[front][right]) == defence) {
